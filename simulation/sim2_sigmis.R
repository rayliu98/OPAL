...
# DGP 2.5 Signal of parameter corresponding to the missed variable varies
...
sig.miss <- seq(0,2,0.2) #control the signal strength of the misspecified parameter
...
alphaM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(sig.miss))) #value of coverage rate for true parameters (focus on the first 21 coefs) 
lengthciM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(sig.miss))) #length of ci (focus on the first 21 coefs)
timecostM <-array(0,c(length(Ms),3,length(ds),length(sig.miss)))#'3' means the output of 'proc.time'

dimnames(alphaM) <- dimnames(lengthciM) <- list(NULL,M_name,md_ci_name,md_basic_name,d_name,as.character(sig.miss))
...
###############################
sfInit(parallel = T,cpus = cpunum,slaveOutfile = "sim_fan_3_sigmis.txt")
sfLibrary(snowfall)
sfLibrary(glmnet)
sfLibrary(ncvreg)
sfLibrary(ncpen)
...
for (dind in 1:length(ds)) {
  ...
  for (nsi in 1:length(ns)) {
    ...
    for (sm in 1:length(sig.miss)) {
      ...
      alphaM[,nsi,,,dind,sm]<-(Reduce("+",Store[1,])/length(Store[1,]))[1:21,,]
      lengthciM[,nsi,,,dind,sm]<-(Reduce("+",Store[2,])/length(Store[2,]))[1:21,,]
      
      timecostM[nsi,,dind,sm]<-timecost.whole[1:3]
      ...
      save.image(file = paste("sim_fan_3_sigmis","_G_",G,"_cpu_",cpunum,".Rdata",sep=""))
    }
  }
}
sfStop()
