...
# DGP 2.6 Local sample size varies
...
bys=1:10 #difference parameter, used for 'seq' function, seq(from,by,length.out), 
...
alphaM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(bys))) #value of coverage rate for true parameters (focus on the first 21 coefs) 
lengthciM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(bys))) #length of ci (focus on the first 21 coefs)
timecostM <-array(0,c(length(Ms),3,length(ds),length(bys)))#'3' means the output of 'proc.time'

dimnames(alphaM) <- dimnames(lengthciM) <- list(NULL,M_name,md_ci_name,md_basic_name,d_name,as.character(bys))
...
###############################
sfInit(parallel = T,cpus = cpunum,slaveOutfile = "sim_fan_3_nby.txt")
sfLibrary(snowfall)
sfLibrary(glmnet)
sfLibrary(ncvreg)
sfLibrary(ncpen)
...
for (dind in 1:length(ds)) {
  ...
  for (nsi in 1:length(ns)) {
    n1 <- ns[nsi]
    M <- round(N/n1)
    for (nby in 1:length(bys)) {
      by <- bys[nby]*10*(n1/1000)^2
      sfExport('by')
      ...
      alphaM[,nsi,,,dind,nby]<-(Reduce("+",Store[1,])/length(Store[1,]))[1:21,,]
      lengthciM[,nsi,,,dind,nby]<-(Reduce("+",Store[2,])/length(Store[2,]))[1:21,,]
      
      timecostM[nsi,,dind,nby]<-timecost.whole[1:3]
      ...
      save.image(file = paste("sim_fan_3_nby","_G_",G,"_cpu_",cpunum,".Rdata",sep=""))
    }
  }
}
sfStop()
