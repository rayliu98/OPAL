# Computational overhead analysis when deep models are used in some sites.
# author: Ray Liu
# date: 2026/02/28
...
#### Notes 
###############################
#### Notes for deep model: 
#1. 'patch_size=32' which is different with 'patch_size=10' used in 'sim1.R';
#2. Both 'Kfold=5' and 'Kfold=2' are considered to measure the time increment caused
#   by the increased folds number of cross-validation
...
Kfold <- 2 #when predicting the probability of in-site observations, how many folds we set?
#cpunum <- c(20,10,8,5) #the setting for the amount of CPUs requested for the cluster
#cpunum <- c(25,10)
#cpunum <- c(1,1)
cpunum=matrix(c(34,25),1,2)
#cpunum=matrix(c(5,5),1,2)

# specific model parameter
MLP_hidden1=c(64,32)
MLP_hidden2=c(64,32,16)
xgb_nrounds1=2
xgb_maxdepth1=6
xgb_nrounds2=8
xgb_maxdepth2=3
...
K.tableM <-array(dim=c(length(Ms),length(ns),2,4,length(d0s),length(ds),length(decays),length(sigs)))
K.tableM.sd <-array(dim=c(length(Ms),length(ns),2,4,length(d0s),length(ds),length(decays),length(sigs))) 
...
dimnames(K.tableM)=dimnames(K.tableM.sd)=list(M_name,n_name,c('elbow','distortion'),c('scad','lasso','glm','mix'),d0_name,d_name,decay_name,sig_name)
...
simfunc <- function(g){
  ...
  for (j in 1:M){
    ...
    ###############################
    #### MLP (para=d*MLP_hidden1+(MLP_hidden1+1)*MLP_hidden2+
    #### (MLP_hidden2+1)*1=11*64+(64+1)*32+(32+1)*1=704+2080+33=2817)
    ###############################
    if(j %in% ml_partition[[2]]){
      Neuralm1 <- keras_model_sequential() %>%
        layer_dense(units = MLP_hidden1[1], activation = 'relu', 
                    input_shape = c(ncol(Xm) - 1)) %>%
        layer_dense(units = MLP_hidden1[2], activation = 'relu') %>%
      ...
      Neuralm1 %>% fit(Xm[,-1], Ym, epochs = epochm[j], batch_size = 32,verbose=0)
      ...
      for (k in 1:Kfold) {
        ...
        Neuralmcv %>% compile(loss = 'binary_crossentropy', 
                              optimizer = 'adam', 
                              metrics = c('accuracy'))
        Neuralmcv %>% fit(Xm[-obs_partition[[k]],-1], Ym[-obs_partition[[k]]], epochs = epochm[j], batch_size = 32,verbose=0)
    ...
    #### (MLP_hidden2+1)*MLP_hidden3+(MLP_hidden3+1)*1=11*64+(64+1)*32+(32+1)*16+(16+1)=704+2080+528+17=3329)
    ###############################
    if(j %in% ml_partition[[3]]){
      Neuralm2 <- keras_model_sequential() %>%
        ...
        layer_dense(units = MLP_hidden2[3], activation = 'relu') %>%
        layer_dense(units = 1, activation = 'sigmoid')
      Neuralm2 %>% compile(loss = 'binary_crossentropy', 
                           optimizer = 'adam', 
                           metrics = c('accuracy'))
      ptm1=proc.time()
      Neuralm2 %>% fit(Xm[,-1], Ym, epochs = epochm[j], batch_size = 32,verbose=0)
  ...
  for (ml in 1:md_ml) {
    for (ml2 in ml_partition[[ml]]) {
      ...
      if(ml==2){
        explainer_keras <- shap$DeepExplainer(modelsMLP[[ml2]],XX[[ml2]][,-1])
        #explainer_keras <- shap$KernelExplainer(modelsMLP[[ml2]],XX[[ml2]][,-1])
        shap_values_keras <- explainer_keras$shap_values(XX[[ml2]][1:100,-1])
        shapleyrecord_mix[ml2,]=apply(shap_values_keras[[1]],2,mean)
        modelsmix[[ml2]]=modelsMLP[[ml2]];MUtildecvmix[[ml2]]=MUtildecvMLP[[ml2]]
      }
      if(ml==3){
        explainer_keras <- shap$DeepExplainer(modelsMLP2[[ml2]],XX[[ml2]][,-1])
        #explainer_keras <- shap$KernelExplainer(modelsMLP[[ml2]],XX[[ml2]][,-1])
        shap_values_keras <- explainer_keras$shap_values(XX[[ml2]][1:100,-1])
        shapleyrecord_mix[ml2,]=apply(shap_values_keras[[1]],2,mean)
        modelsmix[[ml2]]=modelsMLP2[[ml2]];MUtildecvmix[[ml2]]=MUtildecvMLP2[[ml2]]
  ...
  NeuralFull %>% compile(loss = 'binary_crossentropy', 
                         optimizer = 'adam', 
                         metrics = c('accuracy'))
  NeuralFull %>% fit(X[,-1], Y, epochs = 50, batch_size = 32,verbose=0)
  ...
  NeuralFull2 %>% compile(loss = 'binary_crossentropy', 
                          optimizer = 'adam', 
                          metrics = c('accuracy'))
  NeuralFull2 %>% fit(X[,-1], Y, epochs = 50, batch_size = 32,verbose=0)
  ...
  K.table_summary=rbind(cbind(K.table[1:2,],mix=K.table.ml[1:2])) 
        ...
        for (nsi in 1:length(ns)) {
          for (mi in 1:length(Ms)) {
            ...
            for (b in 1:num_batches) {
              ...
              sfInit(parallel = T,cpus = cpunum[nsi,mi],slaveOutfile = "sim1_deep_model.txt")
              sfLibrary(snowfall)
              sfLibrary(glmnet)
              sfLibrary(ncvreg)
              sfLibrary(ncpen)
            ...
            save.image(file = paste("sim1_deep_model","_Kfold_",Kfold,"_cpu_",cpunum[nsi,mi],"_d0_",d0,"_d_",d,"_M_",M,"_n_",n1,".Rdata",sep=""))
            sfStop()
          }
        }
      }
...
#save.image(file = paste("sim1_deep_model_deepShap_test",".Rdata",sep=""))
#save.image(file = paste("sim1_deep_model_kernelShap_batchsize_10",".Rdata",sep=""))
#save.image(file = paste("sim1_deep_model_kernelShap_batchsize_32",".Rdata",sep=""))
#save.image(file = paste("sim1_deep_model_deepShap_batchsize_32",".Rdata",sep=""))
#save.image(file = paste("sim1_deep_model_deepShap_batchsize_32_cv2",".Rdata",sep=""))
# 重点对比：时间，K.table, aac
# batch_size=32节约时间，CV折数减小节约时间