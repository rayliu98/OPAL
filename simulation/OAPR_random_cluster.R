...
#cpunum <- c(25,10)
#cpunum <- c(1,1)
cpunum=matrix(c(25,25,25,25,20,10,10,5),4,2)
#cpunum=matrix(rep(1,8),4,2)
...
md_z_name <- md_b_name
md_OAPR_name = c(paste(c("elbow","distortion",paste("fixed",1:K_fixed_num,sep="_"),"OAP",'EW'),'p',sep="_"),
                 paste(c("random_K",paste("fixed",1:K_fixed_num,sep="_")),'r',sep="_"),
                 paste(c("elbow","distortion",paste("fixed",1:K_fixed_num,sep="_")),'cv',sep="_"))
...
msebM <-array(dim=c(length(Ms),length(ns),md_b,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #MSE for coefficient estimation
msepM <-array(dim=c(length(Ms),length(ns),md_p,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #MSE for probability estimation
accM <-array(dim=c(length(Ms),length(ns),md_a,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #accuracy index
aucM <-array(dim=c(length(Ms),length(ns),md_a,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #auc index
msebM.sd <-array(dim=c(length(Ms),length(ns),md_b,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #MSE for coefficient estimation
msepM.sd <-array(dim=c(length(Ms),length(ns),md_p,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #MSE for probability estimation
accM.sd <-array(dim=c(length(Ms),length(ns),md_a,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #accuracy index
aucM.sd <-array(dim=c(length(Ms),length(ns),md_a,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #auc index
zerorightM <-array(dim=c(length(Ms),length(ns),md_z,md_basic-1,length(d0s),length(ds),length(decays),length(sigs))) #correct zero estimated by penalized methods
zerowrongM <-array(dim=c(length(Ms),length(ns),md_z,md_basic-1,length(d0s),length(ds),length(decays),length(sigs))) #incorrect zero estimated by penalized methods
timecostM <-array(0,c(length(Ms),length(ns),3,length(d0s),length(ds),length(decays),length(sigs)))#'3' means the output of 'proc.time'
timecostMd <-array(0,c(length(Ms),length(ns),md_p,md_basic+1,length(d0s),length(ds),length(decays),length(sigs)))
K.tableM <-array(dim=c(length(Ms),length(ns),5,4,length(d0s),length(ds),length(decays),length(sigs)))
K.tableM.sd <-array(dim=c(length(Ms),length(ns),5,4,length(d0s),length(ds),length(decays),length(sigs))) 
msebM.OAPR <-array(dim=c(length(Ms),length(ns),md_OAPR,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #MSE for coefficient estimation
msepM.OAPR <-array(dim=c(length(Ms),length(ns),md_OAPR,md_basic+1,length(d0s),length(ds),length(decays),length(sigs))) #MSE for probability estimation
accM.OAPR <-array(dim=c(length(Ms),length(ns),md_OAPR,md_basic+1,length(d0s),length(ds),length(decays),length(sigs))) #accuracy index
aucM.OAPR <-array(dim=c(length(Ms),length(ns),md_OAPR,md_basic+1,length(d0s),length(ds),length(decays),length(sigs))) #auc index
msebM.OAPR.sd <-array(dim=c(length(Ms),length(ns),md_OAPR,md_basic,length(d0s),length(ds),length(decays),length(sigs))) #MSE for coefficient estimation
msepM.OAPR.sd <-array(dim=c(length(Ms),length(ns),md_OAPR,md_basic+1,length(d0s),length(ds),length(decays),length(sigs))) #MSE for probability estimation
accM.OAPR.sd <-array(dim=c(length(Ms),length(ns),md_OAPR,md_basic+1,length(d0s),length(ds),length(decays),length(sigs))) #accuracy index
aucM.OAPR.sd <-array(dim=c(length(Ms),length(ns),md_OAPR,md_basic+1,length(d0s),length(ds),length(decays),length(sigs))) #auc index

dimnames(msebM) <- dimnames(msebM.sd) <- list(M_name,n_name,md_b_name,md_basic_name,d0_name,d_name,decay_name,sig_name)
dimnames(msepM) <- dimnames(accM) <- dimnames(aucM) <- dimnames(msepM.sd) <- dimnames(accM.sd) <- dimnames(aucM.sd) <- 
  list(M_name,n_name,md_p_name,md_basic_name,d0_name,d_name,decay_name,sig_name)
dimnames(zerorightM) <- dimnames(zerowrongM) <- list(M_name,n_name,md_z_name,md_basic_name[-3],d0_name,d_name,decay_name,sig_name)
dimnames(timecostMd) <- list(M_name,n_name,md_p_name,c(md_basic_name,'mix'),d0_name,d_name,decay_name,sig_name)
dimnames(K.tableM)=dimnames(K.tableM.sd)=list(M_name,n_name,c('elbow','distortion',"random_K_r",'elbow_cv','distortion_cv'),c('scad','lasso','glm','mix'),d0_name,d_name,decay_name,sig_name)
dimnames(msepM.OAPR) <- dimnames(accM.OAPR) <- dimnames(aucM.OAPR) <- 
  dimnames(msepM.OAPR.sd) <- dimnames(accM.OAPR.sd) <- dimnames(aucM.OAPR.sd) <- 
  list(M_name,n_name,md_OAPR_name,c(md_basic_name,'mix'),d0_name,d_name,decay_name,sig_name)
dimnames(msebM.OAPR) <- dimnames(msebM.OAPR.sd) <- list(M_name,n_name,md_OAPR_name,c(md_basic_name),d0_name,d_name,decay_name,sig_name)
...
simfunc <- function(g){
  ...
  ###############################
  random_partition <- function(M, K){
    group <- c(1:K, sample(1:K, M-K, replace = TRUE))
    sample(group)
  }
  M_for_Kmeans <- 1:K_fixed_num
  K_fixed_sep=floor((M-2)/K_fixed_num)
  K_fixed = M_for_Kmeans*K_fixed_sep
  random_clusters=array(0,c(M,1+K_fixed_num,md_basic+1),
                        dimnames = list(NULL, c("random_K",paste("fixed",K_fixed)),
                                        c("scad","lasso","glm",'mix')))
  K.random.table=matrix(0,1+K_fixed_num,md_basic+1)
  
  for (md in 1:4) {
    for (kseries in 1:(1+K_fixed_num)) {
      if(kseries==1){
        K=sample.int(M,1)
        K.random.table[kseries,md]=K
        group <- random_partition(M,K)
        random_clusters[,1,md]=group
      } else {
        group <- random_partition(M,K_fixed[kseries-1])
        random_clusters[,kseries,md]=group
      }
    }
  }
  for (kseries in 1:K_fixed_num) {
    K.random.table[kseries+1,] <- K_fixed[kseries]
  }
  ###############################
  #### Method OAPR - random selection
  ###############################
  ###############################
  #### glm - random selection
  ###############################
  betahatOAPR.random <- array(0,c(d,1+K_fixed_num,3),dimnames = list(NULL,
                                                                    c("random_K",paste("fixed",K_fixed)),
                                                                    c("scad","lasso","glm")))
  w.random.table <- array(0,c(M,1+K_fixed_num,md_basic),
                          dimnames = list(NULL, c("random_K",paste("fixed",K_fixed)),
                                          c("scad","lasso","glm")))
  for (md in 1:3) {
    for (kseries in 1:(1+K_fixed_num)) {
      res <- betahat_OARFISH(XX,YY,betahat[,md,],MUtildecv[md,],random_clusters[,kseries,md])
      coefs.OAPR <- res$coefs
      #coefs.OAPR[abs(coefs.OAPR)<1e-8] <- 0
      betahatOAPR.random[,kseries,md]=coefs.OAPR # OAPR coefficient estimator with osqp solver
      w.random.table[,kseries,md] <- res$w
    }
  }
  
  ###############################
  #### mix - random selection
  ###############################
  # Record the estimated weights
  wo.random.OAPR <- array(0,c(M,1+K_fixed_num,1),
                   list(NULL,
                        c("random_K",paste("fixed",K_fixed)),
                        c("mix")))
  ...
  for (md in 1:1) {
    ...
    for (kseries in 1:(1+K_fixed_num)) {
      res <- betahat_OARFISH(XX,YY,MUtilde_list=MUtildecvo,cluster_vec=random_clusters[,kseries,4],model_list=model_list)
      wo.random.OAPR[,kseries,md] <- res$w
    }
  }
  sfCat(paste0("Replicate ",g,": random OARP completes"), sep="\n")
  
  ###############################
  #### Clustering - CV prediction loss based
  ###############################
  #### K-selection
  # 1. Temporarily we consider three schemes of selecting K: (1) K=1:M (fixed values); 
  # (2) elbow method; (3) distortion method (invalid! equivalent to 'equal' method).
  # 2. The cross-validation prediction losses (single value) computed at each site 
  # are used for clustering.
  cvloss=matrix(NA,M,4)
  for (md in 1:4) {
    for (m in 1:M) {
      if(md<4){
        cvloss[m,md]=crossprod(YY[[m]]-unlist(MUtildecv[md,m]))/length(YY[[m]])
      } else {
        cvloss[m,md]=crossprod(YY[[m]]-unlist(MUtildecvmix[m]))/length(YY[[m]])
      }
    }
  }
  
  K.cvloss.table <- matrix(0,2+length(K_fixed),4)
  for (md in 1:4) {
    K.cvloss.table[1,md] <- K_selection_new(matrix(cvloss[,md],ncol=1),"elbow",M-2,frac=0.5)
    K.cvloss.table[2,md] <- K_selection_new(matrix(cvloss[,md],ncol=1),"distortion",M-2,distortion_ratio=1)
  }
  for (kseries in 1:length(K_fixed)) {
    K.cvloss.table[kseries+2,] <- K_fixed[kseries]
  }
  dimnames(K.cvloss.table) <- list(
  ...
  #### Implement K-means
  ###############################
  #### glm - CV prediction loss based
  ###############################
  cvloss_clusters <- array(0,c(M,2+K_fixed_num,4),
                           dimnames = list(NULL, c("elbow","distortion",paste("fixed",K_fixed)),
                                           c("scad","lasso","glm",'mix')))
  for (kseries in 1:(2+K_fixed_num)) {
    for (md in 1:4) {
      if(K.cvloss.table[kseries,md]==M){
        cvloss_clusters[,kseries,md] <- 1:M
      } else {
        kmeans_tmp <- KMeans_rcpp(matrix(cvloss[,md],ncol=1), K.cvloss.table[kseries,md])
        cvloss_clusters[,kseries,md] <- kmeans_tmp$clusters
      }
    }
  }
  sfCat(paste0("Replicate ",g,": CV prediction loss based Kmeans completes"), sep="\n")
  
  ###############################
  #### glm - CV prediction loss based
  ###############################
  betahatOAPR.cvloss <- array(0,c(d,2+K_fixed_num,3),dimnames = list(NULL,
                                                                     c("elbow","distortion",paste("fixed",K_fixed)),
                                                                     c("scad","lasso","glm")))
  w.cvloss.table <- array(0,c(M,2+K_fixed_num,md_basic),
                          dimnames = list(NULL, c("elbow","distortion",paste("fixed",K_fixed)),
                                          c("scad","lasso","glm")))
  for (md in 1:3) {
    for (kseries in 1:(2+K_fixed_num)) {
      res <- betahat_OARFISH(XX,YY,betahat[,md,],MUtildecv[md,],cvloss_clusters[,kseries,md])
      coefs.OAPR <- res$coefs
      #coefs.OAPR[abs(coefs.OAPR)<1e-8] <- 0
      betahatOAPR.cvloss[,kseries,md]=coefs.OAPR # OAPR coefficient estimator with osqp solver
      w.cvloss.table[,kseries,md] <- res$w
    }
  }
  
  ###############################
  #### mix - CV prediction loss based
  ###############################
  # Record the estimated weights
  wo.cvloss.OAPR <- array(0,c(M,2+K_fixed_num,1),
                          list(NULL,
                               c("elbow","distortion",paste("fixed",K_fixed)),
                               c("mix")))
  ...
  for (md in 1:1) {
    if(md==1){model_list=modelsmix;MUtildecvo=MUtildecvmix}
    for (kseries in 1:(2+K_fixed_num)) {
      res <- betahat_OARFISH(XX,YY,MUtilde_list=MUtildecvo,cluster_vec=cvloss_clusters[,kseries,4],model_list=model_list)
      wo.cvloss.OAPR[,kseries,md] <- res$w
    }
  }
  sfCat(paste0("Replicate ",g,": cvloss OARP completes"), sep="\n")
  
  ###############################
  #### Clustering - parameter / Shapley based
  ###############################
  #### K-selection
  ...
  K.table <- matrix(0,2+length(K_fixed),3)
  for (md in 1:3) {
    ptm=proc.time()
    ...
    ptm1=proc.time()
    K.table[1,md] <- K_selection_new(betahat_s[,,md],"elbow",M-2,frac=0.5)
    time.k_selection[1,md]=(proc.time()-ptm1)[3]
    
    ptm2=proc.time()
    K.table[2,md] <- K_selection_new(betahat_s[,,md],"distortion",M-2,distortion_ratio=1)
    time.k_selection[2,md]=(proc.time()-ptm2)[3]
  }
  for (kseries in 1:length(K_fixed)) {
    K.table[kseries+2,] <- K_fixed[kseries]
  }
  ...
  K.table.ml <- array(0,c(2+length(K_fixed),1),list(
    c("elbow","distortion",paste("fixed",K_fixed)),
    c("mix")))
  
  for (md in 1:1) {
    ptm1=proc.time()
    K.table.ml[1,md] <- K_selection_new(shapley_s[[md]],"elbow",M-2,frac=0.5)
    time.k_selection[1,4]=(proc.time()-ptm1)[3]
    ptm2=proc.time()
    K.table.ml[2,md] <- K_selection_new(shapley_s[[md]],"distortion",M-2,distortion_ratio=1)
    time.k_selection[2,4]=(proc.time()-ptm2)[3]
  }
  #sum(is.na(K.table.ml))
  for (kseries in 1:length(K_fixed)) {
    K.table.ml[kseries+2,] <- K_fixed[kseries]
  }
  ...
  kmeans_clusters <- array(0,c(M,2+length(K_fixed),3),
                           dimnames = list(NULL, c("elbow","distortion",paste("fixed",K_fixed)),
                                           c("scad","lasso","glm")))
  ...
  #### mix
  ###############################
  kmeans_other_clusters <- array(0,c(M,2+length(K_fixed),1),
                                 dimnames = list(NULL,c("elbow","distortion",paste("fixed",K_fixed)),
                                                 c("mix")))
  for (kseries in 1:(2+length(K_fixed))) {
    for (md in 1:1) {
  ...
  #### glm
  ###############################
  betahatOAPR <- array(0,c(d,2+length(K_fixed),3),dimnames = list(NULL,
                                                                  c("elbow","distortion",paste("fixed",K_fixed)),
                                                                  c("scad","lasso","glm")))
  iter.center_opt <- array(0,c(2+length(K_fixed),3),dimnames = 
                             list(c("elbow","distortion",paste("fixed",K_fixed)),
                                  c("scad","lasso","glm")))
  # Record the estimated weights
  w.table <- array(0,c(M,2+length(K_fixed),3),dimnames = 
  ...
  ###############################
  # Record the estimated weights
  wo.OAPR <- array(0,c(M,2+length(K_fixed),1),
                   list(NULL,
                        c("elbow","distortion",paste("fixed",K_fixed)),
                        c("mix")))
  ...
  md_b_name.otherOAPR <- c(paste(c("random_K",paste("fixed",K_fixed,sep="_")),"r",sep="_"),
                       paste(c("elbow","distortion",paste("fixed",K_fixed,sep="_")),"cv",sep="_"))
  md_b.otherOAPR=length(md_b_name.otherOAPR)
  betahat_all.otherOAPR <- array(dim = c(d,md_b.otherOAPR,md_basic),dimnames = list(NULL,md_b_name.otherOAPR,c("scad","lasso","glm")))
  
  for (md in 1:3) {
    # fill betahat_all
    betahat_all[,1:(2+length(K_fixed)),md] <- betahatOAPR[,,md] # OAPR
    betahat_all[,(2+length(K_fixed))+1,md] <- betahatOAP[,md] # OAP
    betahat_all[,(2+length(K_fixed))+2,md] <- betahatEW[,md] # EW
    betahat_all[,(2+length(K_fixed))+3,md] <- betahat[,md,1] # Partial
    betahat_all[,(2+length(K_fixed))+4,md] <- betahatFull[,md] # Full
    
    betahat_all.otherOAPR[,1:(1+length(K_fixed)),md]=betahatOAPR.random[,,md]
    betahat_all.otherOAPR[,(2+length(K_fixed)):md_b.otherOAPR,md]=betahatOAPR.cvloss[,,md]
  }
  ...
  storelossb.otherOAPR <- apply(betahat_all.otherOAPR,c(2,3),function(x){sum((x - theta_star[1:d])^2)})
  ...
  pred_prob1 <- array(0,c(nrow(Xst),2+length(K_fixed)+2,3))
  pred_probs1 <- array(0,c(nrow(Xst),M,3))
  pred_prob1.otherOAPR <- array(0,c(nrow(Xst),md_b.otherOAPR,3))
  storelossp.otherOAPR=storelossaac.otherOAPR=storelossauc.otherOAPR=
    matrix(nrow=md_b.otherOAPR,ncol=md_basic+1,dimnames = list(md_b_name.otherOAPR,c("scad","lasso","glm","mix")))
  ...
  for (mdp in 1:3) {
    for (kseries in 1:(2+length(K_fixed)+2)) {
      ...
      if(kseries<(2+length(K_fixed)+1)){
        pred_prob1[,kseries,mdp] <- pred_probs1[,,mdp]%*%w.table[,kseries,mdp]
      }
      if(kseries==(2+length(K_fixed)+1)){
        pred_prob1[,kseries,mdp] <- pred_probs1[,,mdp]%*%w.OAP[,mdp]
      }
      if(kseries==(2+length(K_fixed)+2)){
        pred_prob1[,kseries,mdp] <- pred_probs1[,,mdp]%*%w0
      }
      time.pred_OPAL[kseries,mdp]=(proc.time()-ptm)[3]
    }
  }
  for (mdp in 1:3) {
    for (kseries in 1:md_b.otherOAPR) {
      if(kseries<(2+length(K_fixed))){
        pred_prob1.otherOAPR[,kseries,mdp] <- pred_probs1[,,mdp]%*%w.random.table[,kseries,mdp]
      } else {
        pred_prob1.otherOAPR[,kseries,mdp] <- pred_probs1[,,mdp]%*%w.cvloss.table[,kseries-(1+K_fixed_num),mdp]
      }
    }
  }
  
  for (mdp in 1:3) {
    storelossp[(md_p-(2+length(K_fixed)+2)+1):md_p,mdp] <- colMeans((sweep(-pred_prob1[,,mdp],1,probst,"+"))^2)
    storelossp.otherOAPR[1:md_b.otherOAPR,mdp] <- colMeans((sweep(-pred_prob1.otherOAPR[,,mdp],1,probst,"+"))^2)
    
    pred_y <- pred_prob1[,,mdp]>=0.5
    pred_y.otherOAPR=pred_prob1.otherOAPR[,,mdp]>=0.5
    if(any(pred_prob1[,,mdp]==0.5,na.rm=T)){
      ...
      pred_y.otherOAPR[pred_prob1.otherOAPR[,,mdp]==0.5&!is.na(pred_prob1.otherOAPR[,,mdp])] <- sample(c(FALSE,TRUE),sum(pred_prob1.otherOAPR[,,mdp]==0.5),replace=T)
    }
    storelossaac[(md_p-(2+length(K_fixed)+2)+1):md_p,mdp] <- colMeans(abs(sweep(-pred_y,1,Yst,"+")),na.rm=T)
    storelossaac.otherOAPR[1:md_b.otherOAPR,mdp] <- colMeans(abs(sweep(-pred_y.otherOAPR,1,Yst,"+")),na.rm=T)
    for (j in 1:dim(pred_prob1)[2]) {
      pred_roc <- roc(factor(Yst,levels = c(0,1)),as.numeric(pred_prob1[,j,mdp]),quiet=T)
      pred_auc <- auc(pred_roc)
      storelossauc[(md_p-(2+length(K_fixed)+2))+j,mdp] <- pred_auc
    }
    for (j in 1:dim(pred_prob1.otherOAPR)[2]) {
      pred_roc <- roc(factor(Yst,levels = c(0,1)),as.numeric(pred_prob1.otherOAPR[,j,mdp]),quiet=T)
      pred_auc <- auc(pred_roc)
      storelossauc.otherOAPR[j,mdp] <- pred_auc
    }
  }
  ...
  pred_prob <- matrix(0,nrow(Xst),md_p-(2+length(K_fixed)+2)-md_b)
  pred_prob.otherOAPR <- matrix(0,nrow(Xst),md_b.otherOAPR)
  ...
  for (kseries in 1:(2+length(K_fixed)+2)) {
    ptm=proc.time()
    if(kseries<(2+length(K_fixed)+1)){
      pred_prob[,kseries+8] <- pred_probs[,]%*%wo.OAPR[,kseries,1]
    }
    if(kseries==(2+length(K_fixed)+1)){
      pred_prob[,kseries+8] <- pred_probs[,]%*%wo.OAP[,1]
    }
    if(kseries==(2+length(K_fixed)+2)){
      pred_prob[,kseries+8] <- rowMeans(pred_probs[,])
    }
  ...
  for (kseries in 1:md_b.otherOAPR) {
    if(kseries<(2+length(K_fixed))){
      pred_prob.otherOAPR[,kseries] <- pred_probs[,]%*%wo.random.OAPR[,kseries,1]
    } else {
      pred_prob.otherOAPR[,kseries] <- pred_probs[,]%*%wo.cvloss.OAPR[,kseries-(1+K_fixed_num),1]
    }
  }
  
  #Two evaluation criteria: probst for Risk, and Yst for accuracy
  storelossp[(md_b+1):(md_p-(2+length(K_fixed)+2)),3] <- colMeans((sweep(-pred_prob,1,probst,"+"))^2)
  storelossp.otherOAPR[,4] <- colMeans((sweep(-pred_prob.otherOAPR,1,probst,"+"))^2)
  pred_y <- pred_prob>=0.5
  pred_y.otherOAPR=pred_prob.otherOAPR>=0.5
  if(any(pred_prob==0.5,na.rm=T)){
    pred_y[pred_prob==0.5&!is.na(pred_prob)] <- sample(c(FALSE,TRUE),sum(pred_prob==0.5),replace=T)
    pred_y.otherOAPR[pred_prob.otherOAPR==0.5&!is.na(pred_prob.otherOAPR)] <- sample(c(FALSE,TRUE),sum(pred_prob.otherOAPR==0.5),replace=T)
  }
  storelossaac[(md_b+1):(md_p-(2+length(K_fixed)+2)),3] <- colMeans(abs(sweep(-pred_y,1,Yst,"+")),na.rm=T)
  storelossaac.otherOAPR[1:md_b.otherOAPR,4] <- colMeans(abs(sweep(-pred_y.otherOAPR,1,Yst,"+")),na.rm=T)
  for (j in 1:(md_p-(2+length(K_fixed)+2)-md_b)) {
    pred_roc <- roc(factor(Yst,levels = c(0,1)),as.numeric(pred_prob[,j]),quiet=T)
    pred_auc <- auc(pred_roc)
    storelossauc[md_b+j,3] <- pred_auc
  }
  for (j in 1:md_b.otherOAPR) {
    pred_roc <- roc(factor(Yst,levels = c(0,1)),as.numeric(pred_prob.otherOAPR[,j]),quiet=T)
    pred_auc <- auc(pred_roc)
    storelossauc.otherOAPR[j,4] <- pred_auc
  }
  
  storelossb.OAPR=rbind(storelossb[1:(2+2+K_fixed_num),],storelossb.otherOAPR)
  storelossp.OAPR=rbind(cbind(storelossp[(md_p-(2+2+K_fixed_num)+1):md_p,],
                                mix=storelossp[(md_p-2*(2+2+K_fixed_num)+1):(md_p-(2+2+K_fixed_num)),3]),
                          storelossp.otherOAPR)
  storelossaac.OAPR=rbind(cbind(storelossaac[(md_p-(2+2+K_fixed_num)+1):md_p,],
                                mix=storelossaac[(md_p-2*(2+2+K_fixed_num)+1):(md_p-(2+2+K_fixed_num)),3]),
                          storelossaac.otherOAPR)
  storelossauc.OAPR=rbind(cbind(storelossauc[(md_p-(2+2+K_fixed_num)+1):md_p,],
              mix=storelossauc[(md_p-2*(2+2+K_fixed_num)+1):(md_p-(2+2+K_fixed_num)),3]),
        storelossauc.otherOAPR)
  ...
  time.after_weight[1:(2+K_fixed_num),]=time.after_weight[1:(2+K_fixed_num),]+time.weight.OAPR
  time.after_weight[2+K_fixed_num+1,]=time.after_weight[2+K_fixed_num+1,]+time.weight.OAP
  time.after_weight[2+K_fixed_num+2,1:3]=time.after_weight[2+K_fixed_num+2,1:3]+time.ew
  
  timeTable[1:(2+K_fixed_num+2),1:3]=time.after_weight[1:(2+K_fixed_num+2),1:3]+time.pred_OPAL_coef[1:(2+K_fixed_num+2),1:3]
  timeTable[(2+K_fixed_num+2)+1,1:3]=apply(time.fitting,2,max)[1:3]+time.pred_M[1,]
  timeTable[(2+K_fixed_num+2)+2,1:3]=time.full_glm+time.pred_OPAL_coef[(2+K_fixed_num+2)+2,1:3]
  timeTable[(2+K_fixed_num+2)+3,4]=time.full_mlp
  timeTable[(2+K_fixed_num+2)+4,4]=time.full_mlp2
  timeTable[(2+K_fixed_num+2)+5,4]=time.full_xgb
  timeTable[(2+K_fixed_num+2)+6,4]=time.full_xgb2
  timeTable[(2+K_fixed_num+2+7):(2+K_fixed_num+2+10),4]=time.fitting[,4][unname(sapply(ml_partition, function(x) x[1]))[-1]]
  timeTable[(2+K_fixed_num+2+3):(2+K_fixed_num+2+10),4]=timeTable[(2+K_fixed_num+2+3):(2+K_fixed_num+2+10),4]+time.pred_ml
  timeTable[(2+K_fixed_num+2+11):((2+K_fixed_num+2)*2+10),4]=time.after_weight[1:(2+K_fixed_num+2),4]+time.pred_mix+max(time.pred_M_ml)
  timeTable[((2+K_fixed_num+2)*2+11):((2+K_fixed_num+2)*3+10),1:3]=time.after_weight[1:(2+K_fixed_num+2),1:3]+time.pred_OPAL+matrix(apply(time.pred_M,2,max),2+K_fixed_num+2,3,byrow=T)
  
  # save the data-driven K by methods 'elbow' and 'distortion'
  K.table_summary=rbind(cbind(K.table[1:2,],mix=K.table.ml[1:2]),random=K.random.table[1,],
                        elbow_cv=K.cvloss.table[1,],distortion.cv=K.cvloss.table[2,]) 
  ...
  return(list(storelossb=storelossb,storelossp=storelossp,storelossaac=storelossaac,
              ...
              K.table=K.table_summary,storelossb.OAPR=storelossb.OAPR,
              storelossp.OAPR=storelossp.OAPR,storelossaac.OAPR=storelossaac.OAPR,
              storelossauc.OAPR=storelossauc.OAPR))
}
...
###############################
# d0ind=dind=decayi=nsi=sigi=mi=g=1
      ...
      for (sigi in 1:length(sigs)){
        for (nsi in 1:length(ns)) {
          for (mi in 1:length(Ms)) {
            sfInit(parallel = T,cpus = cpunum[nsi,mi],slaveOutfile = "OAPR_random_cluster.txt")
            sfLibrary(snowfall)
            sfLibrary(glmnet)
            sfLibrary(ncvreg)
            sfLibrary(ncpen)
            ...
            msebM[mi,nsi,,,d0ind,dind,decayi,sigi]<-Reduce("+",Store[1,])/length(Store[1,])
            msepM[mi,nsi,,,d0ind,dind,decayi,sigi]<-Reduce("+",Store[2,])/length(Store[2,])
            accM[mi,nsi,,,d0ind,dind,decayi,sigi]<-Reduce("+",Store[3,])/length(Store[3,])
            aucM[mi,nsi,,,d0ind,dind,decayi,sigi]<-Reduce("+",Store[4,])/length(Store[4,])
            ...
            msebM.OAPR[mi,nsi,,,d0ind,dind,decayi,sigi]<-Reduce("+",Store[9,])/G
            msepM.OAPR[mi,nsi,,,d0ind,dind,decayi,sigi]<-Reduce("+",Store[10,])/G
            accM.OAPR[mi,nsi,,,d0ind,dind,decayi,sigi]<-Reduce("+",Store[11,])/G
            aucM.OAPR[mi,nsi,,,d0ind,dind,decayi,sigi]<-Reduce("+",Store[12,])/G
            msebM.OAPR.sd[mi,nsi,,,d0ind,dind,decayi,sigi]<-sqrt(Reduce("+",lapply(Store[9,],function(X){(X-msebM.OAPR[mi,nsi,,,d0ind,dind,decayi,sigi])^2}))/(G-1)/G)
            msepM.OAPR.sd[mi,nsi,,,d0ind,dind,decayi,sigi]<-sqrt(Reduce("+",lapply(Store[10,],function(X){(X-msepM.OAPR[mi,nsi,,,d0ind,dind,decayi,sigi])^2}))/(G-1)/G)
            accM.OAPR.sd[mi,nsi,,,d0ind,dind,decayi,sigi]<-sqrt(Reduce("+",lapply(Store[11,],function(X){(X-accM.OAPR[mi,nsi,,,d0ind,dind,decayi,sigi])^2}))/(G-1)/G)
            aucM.OAPR.sd[mi,nsi,,,d0ind,dind,decayi,sigi]<-sqrt(Reduce("+",lapply(Store[12,],function(X){(X-aucM.OAPR[mi,nsi,,,d0ind,dind,decayi,sigi])^2}))/(G-1)/G)
            ...
            save.image(file = paste("OAPR_random_cluster","_cpu_",cpunum[nsi,mi],"_d0_",d0,"_d_",d,"_M_",M,"_n_",n1,".Rdata",sep=""))
            sfStop()
          }
        }
      }
