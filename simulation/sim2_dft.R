...
# DGP 2.3 Degree of freedom of t-distribution varies
...
df.ts <- seq(30,3,-3) #control the degree of freedom of t distribution
...
alphaM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(df.ts))) #value of coverage rate for true parameters (focus on the first 21 coefs) 
lengthciM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(df.ts))) #length of ci (focus on the first 21 coefs)
timecostM <-array(0,c(length(Ms),3,length(ds),length(df.ts)))#'3' means the output of 'proc.time'

dimnames(alphaM) <- dimnames(lengthciM) <- list(NULL,M_name,md_ci_name,md_basic_name,d_name,as.character(df.ts))
...
###############################
sfInit(parallel = T,cpus = cpunum,slaveOutfile = "sim_fan_3_dft.txt")
sfLibrary(snowfall)
sfLibrary(glmnet)
sfLibrary(ncvreg)
sfLibrary(ncpen)
...
for (dind in 1:length(ds)) {
  ...
  for (nsi in 1:length(ns)) {
    ...
    for (dft in 1:length(df.ts)) {
      ...
      alphaM[,nsi,,,dind,dft]<-(Reduce("+",Store[1,])/length(Store[1,]))[1:21,,]
      lengthciM[,nsi,,,dind,dft]<-(Reduce("+",Store[2,])/length(Store[2,]))[1:21,,]
      
      timecostM[nsi,,dind,dft]<-timecost.whole[1:3]
      ...
      save.image(file = paste("sim_fan_3_dft","_G_",G,"_cpu_",cpunum,".Rdata",sep=""))
    }
  }
}
sfStop()
