...
corrxs <- seq(0,0.9,0.1) #control the correlation between the covariates in design matrix
...
alphaM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(corrxs))) #value of coverage rate for true parameters (focus on the first 21 coefs) 
lengthciM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(corrxs))) #length of ci (focus on the first 21 coefs)
timecostM <-array(0,c(length(Ms),3,length(ds),length(corrxs)))#'3' means the output of 'proc.time'

dimnames(alphaM) <- dimnames(lengthciM) <- list(NULL,M_name,md_ci_name,md_basic_name,d_name,as.character(corrxs))
...
simfunc <- function(g){
  ...
  # dimension, kmeans, lower/upper bound, b1y_schurO1_type1
  ...
  # dimension, lower/upper bound, b1y_schurO1_type1
...
###############################
sfInit(parallel = T,cpus = cpunum,slaveOutfile = "sim_fan_3_corr.txt")
sfLibrary(snowfall)
sfLibrary(glmnet)
sfLibrary(ncvreg)
sfLibrary(ncpen)
...
for (dind in 1:length(ds)) {
  ...
  for (nsi in 1:length(ns)) {
    ...
    for (co in 1:length(corrxs)) {
      ...
      alphaM[,nsi,,,dind,co]<-(Reduce("+",Store[1,])/length(Store[1,]))[1:21,,]
      lengthciM[,nsi,,,dind,co]<-(Reduce("+",Store[2,])/length(Store[2,]))[1:21,,]
      
      timecostM[nsi,,dind,co]<-timecost.whole[1:3]
      ...
      save.image(file = paste("sim_fan_3_corr","_G_",G,"_cpu_",cpunum,".Rdata",sep=""))
    }
  }
}
sfStop()
