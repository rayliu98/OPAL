...
# DGP 2.4 Lambda in exponential distribution varies
...
rare <- 0.5 #control the proportion of positive labels (to produce unbalanced data)
df.t <- 0 #control the degree of freedom of t distribution
lambda.es <- 1/seq(0.25,2.5,0.25) #control the lambda of exponential distribution
...
alphaM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(lambda.es))) #value of coverage rate for true parameters (focus on the first 21 coefs) 
lengthciM <-array(dim=c(21,length(Ms),md_ci,md_basic,length(ds),length(lambda.es))) #length of ci (focus on the first 21 coefs)
timecostM <-array(0,c(length(Ms),3,length(ds),length(lambda.es)))#'3' means the output of 'proc.time'

dimnames(alphaM) <- dimnames(lengthciM) <- list(NULL,M_name,md_ci_name,md_basic_name,d_name,as.character(lambda.es))

simfunc <- function(g){
  sfCat(paste0("Replicate ",g,": starts"), sep="\n")
  set.seed(2124+g) #another seed
...
###############################
sfInit(parallel = T,cpus = cpunum,slaveOutfile = "sim_fan_3_lambdae.txt")
sfLibrary(snowfall)
sfLibrary(glmnet)
sfLibrary(ncvreg)
sfLibrary(ncpen)
...
for (dind in 1:length(ds)) {
  ...
  for (nsi in 1:length(ns)) {
    ...
    for (lame in 1:length(lambda.es)) {
      lambda.e <- lambda.es[lame]
      sfExport('lambda.e')
      ...
      alphaM[,nsi,,,dind,lame]<-(Reduce("+",Store[1,])/length(Store[1,]))[1:21,,]
      lengthciM[,nsi,,,dind,lame]<-(Reduce("+",Store[2,])/length(Store[2,]))[1:21,,]
      
      timecostM[nsi,,dind,lame]<-timecost.whole[1:3]
      ...
      save.image(file = paste("sim_fan_3_lambdae","_G_",G,"_cpu_",cpunum,".Rdata",sep=""))
    }
  }
}
sfStop()
