...
#cpunum <- c(25,10)
#cpunum <- c(1,1)
#cpunum=matrix(c(34,34,34,34,34,25,25,15,8,4,20,15,10,4,2),5,3)
cpunum=matrix(c(34,34,34,34,25,25,25,25,20,20,20,20),4,3)
...
simfunc <- function(g){
  ...
  time.after_weight[1:20,]=time.after_weight[1:20,]+time.weight.OAPR
  time.after_weight[21,]=time.after_weight[21,]+time.weight.OAP
  time.after_weight[22,1:3]=time.after_weight[22,1:3]+time.ew
  
  timeTable[1:22,1:3]=time.after_weight[1:22,1:3]+time.pred_OPAL_coef[1:22,1:3]
  timeTable[23,1:3]=apply(time.fitting,2,max)[1:3]+time.pred_M[1,]
  timeTable[24,1:3]=time.full_glm+time.pred_OPAL_coef[24,1:3]
  timeTable[25,4]=time.full_mlp
  timeTable[26,4]=time.full_mlp2
  timeTable[27,4]=time.full_xgb
  timeTable[28,4]=time.full_xgb2
  timeTable[29:32,4]=time.fitting[,4][unname(sapply(ml_partition, function(x) x[1]))[-1]]
  timeTable[25:32,4]=timeTable[25:32,4]+time.pred_ml
  timeTable[33:54,4]=time.after_weight[1:22,4]+time.pred_mix+max(time.pred_M_ml)
  timeTable[55:76,1:3]=time.after_weight[1:22,1:3]+time.pred_OPAL+matrix(apply(time.pred_M,2,max),22,3,byrow=T)
  
  # save the data-driven K by methods 'elbow' and 'distortion'
  K.table_summary=cbind(K.table[1:2,],mix=K.table.ml[1:2]) 
  ...
  return(list(storelossb=storelossb,storelossp=storelossp,storelossaac=storelossaac,
              storelossauc=storelossauc,zeror=zeror,zerow=zerow,timeTable=timeTable,K.table=K.table_summary))
}
        ...
        for (nsi in 1:length(ns)) {
          for (mi in 1:length(Ms)) {
            ...
            for (b in 1:num_batches) {
              ...
              sfInit(parallel = T,cpus = cpunum[nsi,mi],slaveOutfile = "OAPR_sensitivity_G.txt")
              sfLibrary(snowfall)
              sfLibrary(glmnet)
              sfLibrary(ncvreg)
              sfLibrary(ncpen)
            ...
            save.image(file = paste("OAPR_sensitivity_G","_cpu_",cpunum[nsi,mi],"_d0_",d0,"_d_",d,"_M_",M,"_n_",n1,".Rdata",sep=""))
            sfStop()
          }
        }
      }
