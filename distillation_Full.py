...
mu_test=np.zeros((2,np.shape(Y_test)[0])) 
...
teacher.fit(X, Y, epochs=epochm[0], batch_size=batch_sizem[0],verbose=0)
...
distiller.fit(X, Y, epochs=epochm[1], batch_size=batch_sizem[1],verbose=0)
# lossno,loss2,accuracyno,accuracy2=distiller.evaluate(X_test,Y_test)

mu_test[0,:]=distiller.predict(X_test)[:,0]
...
mu_test[1,:]=distiller.predict(X_test)[:,0]
...
# Save as a .pkl file
with open("cnn_arrays_Full.pkl", "wb") as file:
